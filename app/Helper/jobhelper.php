<?php
use App\Models\Ssjobs;
function buildJobArray() {
    $jobs = array();
    $jobst = Ssjobs::where('status', 1)->get();
    $i = 0;

    foreach($jobst as $jobsu){
        $jobs[$i] = new Job;
        $jobs[$i]->id = $jobsu->job_id;
        $jobs[$i]->name = $jobsu->job_name;
		$jobs[$i]->short = $jobsu->job_short;
        $jobs[$i]->start1 = $jobsu->job_start;
        $jobs[$i]->end1 = $jobsu->job_end;
        $jobs[$i]->start2 = $jobsu->job_start2;
        $jobs[$i]->end2 = $jobsu->job_end2;
		$jobs[$i]->hours = $jobsu->job_hours;
		$jobs[$i]->dept = $jobsu->job_parent;
